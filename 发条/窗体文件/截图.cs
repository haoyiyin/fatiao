...
namespace fatiao
{
    ...
    {
        string path = Environment.CurrentDirectory + @".\素材库\";//设置打开路径的目录
        Bitmap bit;//字段
        int x;//字段
        int y;//字段
        int xx;//字段
        ...
        private void 截图_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Space && !e.Control)
            {
                if (xx > x || yy > y) //判断 宽度 或高度 是否为0
                {
                    ...
                    发条.发条窗口.fatiao.Capture((int)((x+3) * 分辨率缩放.ScaleX), (int)((y+3) * 分辨率缩放.ScaleX), (int)((xx-3) * 分辨率缩放.ScaleX), (int)((yy-3) * 分辨率缩放.ScaleX), path + tpName + ".bmp");
                    switch (发条.发条窗口.参数窗口)
                    {
                        case "查找图片":
                            发条.发条窗口.查找图片.textBox3.Text = tpName + ".bmp";
                            发条.发条窗口.查找图片.label19.Visible = false;
                            ...
                            break;
                        case "点击图片":
                            发条.发条窗口.点击图片.label11.Text = tpName + ".bmp";
                            发条.发条窗口.点击图片.label19.Visible = false;
                            img = Image.FromFile(path + tpName + ".bmp");
                            bmp = new Bitmap(img);
                            img.Dispose();
                            发条.发条窗口.点击图片.pictureBox1.Image = bmp;
                            break;
                        case "用户输入窗":
                            发条.发条窗口.参数窗口 =  tpName + ".bmp";
                            break;
                        case "询问输入":
                            发条.发条窗口.参数窗口 = tpName + ".bmp";
                            break;
                ...
                if (xx > x || yy > y) //判断 宽度 或高度 是否为0
                {
                    ...
                    switch (发条.发条窗口.参数窗口)
                    {
                        case "查找图片":
                            ...
                            发条.发条窗口.查找图片.label19.Visible = true;
