...
namespace fatiao
{
    public partial class 用户输入窗 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (textBox1.Visible)
            {
                发条.发条窗口.输入变量值 = textBox1.Text;
                发条.发条窗口.快捷命令输入变量值 = textBox1.Text;
            }
            else
            {
                发条.发条窗口.输入变量值 = dateTimePicker1.Text;
                发条.发条窗口.快捷命令输入变量值 = dateTimePicker1.Text;
            }
            Console.WriteLine(发条.发条窗口.快捷命令输入变量值);
            Close();
        }
        ...
        { 
            ...
            if (发条.发条窗口.参数窗口 != "用户输入窗")
            {
                textBox1.Text = 发条.发条窗口.参数窗口;
            }
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    for (int i = 0; i < str.Length; i++)//查找数组所有数据
                    {
                        ...
                        textBox1.Text += 多图片;//自动填入所选图片目录和名字
                    }
                    textBox1.Text = textBox1.Text.Substring(0, textBox1.Text.Length - 1);//删除最后一个多余的"|"
                }
                catch//如果报错
                {
